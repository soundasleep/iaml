 ...
 *
 */
public class RefreshDataStores {
	...
	public void create(EObject model) throws InferenceException {
        ...
        workingMemory.addEventListener( new WorkingMemoryEventListener() {
			...
			public void objectInserted(ObjectInsertedEvent obj) {
				...
				// FileDomainStore: insert properties file
				// (perhaps not needed)
				/*
				if (obj.getObject() instanceof FileDomainStore) {
					FileDomainStore a = (FileDomainStore) obj.getObject();
					try {
						workingMemory.insert( a.getPropertiesFile() );
					} catch (FileNotFoundException e) {
						throw new FactException(e);
	...
	private RuleBase readRule() throws Exception {
		...
		for (String ruleFile : getRuleFiles()) {
			...
			//read in the source
			Reader source = new InputStreamReader( RefreshDataStores.class.getResourceAsStream( ruleFile ) );
