...
/**
 * When a DomainIterator is connected to a Form by a
 * SetWire, Labels are created, allowing the object to be <em>viewed</em>
 * but not saved.
 *
 * @author jmwright
 * @example CastNode
 * 		Using a {@model CastNode} to cast a {@model Value} from
 ...
 *		{@model DecisionNode} named 'failed?', which will return true if
 *		the {@model DataFlowEdgesSource data source} could not be cast successfully.
 * @implementation DecisionNode
 * 		A {@model DecisionNode} named 'failed?' will return <code>true</code> if
 ...
 * 		the incoming {@model CastNode} (through a {@model DataFlowEdge}) could
 * 		not be cast correctly.
 * @example CastNode,Value,InputTextField
