 ...
 * @author jmwright
 * @example Gate
 * 		Using an exit {@model Gate} to prevent access outside of a {@model Scope}
 * 		without first viewing an advertisement {@model Frame}.
 * @implementation Gate
 * 		If a {@model Session} contains an exit {@model Gate}, all incoming
 * 		{@model Condition Conditions} must be <code>false</code> before the
