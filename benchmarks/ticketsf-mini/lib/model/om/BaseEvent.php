...
abstract class BaseEvent extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the tickets_left field.
	 * Note: this column has a database default value of: 0
	 * @var        int
	 */
	protected $tickets_left;
	...
	/**
	 * @var        Criteria The criteria used to select the current contents of collTickets.
	 */
	...
	/**
	 * Get the [tickets_left] column value.
	 * 
	 * @return     int
	 */
	...
	public function setVenue($v)
	{
		...
		if ($this->venue !== $v) {
			$this->venue = $v;
			$this->modifiedColumns[] = EventPeer::VENUE;
		}
	...
	/**
	 * Set the value of [tickets_left] column.
	 * 
	 * @param      int $v new value
	 * @return     Event The current object (for fluent API support)
	 */
	public function setTicketsLeft($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}

		if ($this->tickets_left !== $v || $this->isNew()) {
			$this->tickets_left = $v;
			$this->modifiedColumns[] = EventPeer::TICKETS_LEFT;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->venue = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->date = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->tickets_left = ($row[$startcol + 5] !== null) ? (int) $row[$startcol + 5] : null;
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[3] => $this->getVenue(),
			$keys[4] => $this->getDate(),
			$keys[5] => $this->getTicketsLeft(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setVenue($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setDate($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setTicketsLeft($arr[$keys[5]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(EventPeer::VENUE)) $criteria->add(EventPeer::VENUE, $this->venue);
		if ($this->isColumnModified(EventPeer::DATE)) $criteria->add(EventPeer::DATE, $this->date);
		if ($this->isColumnModified(EventPeer::TICKETS_LEFT)) $criteria->add(EventPeer::TICKETS_LEFT, $this->tickets_left);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setTicketsLeft($this->tickets_left);
	...
	/**
	 * Clears out the collTickets collection (array).
	 *
	...
	/**
	 * Initializes the collTickets collection (array).
	 *
	 ...
	 * related Tickets from storage. If this Event is new, it will return
		...
		if ($this->collTickets === null) {
			if ($this->isNew()) {
			   $this->collTickets = array();
				...
				$criteria->add(TicketPeer::EVENT_ID, $this->id);
	...
	public function addTicket(Ticket $l)
	{
		...
		if (!in_array($l, $this->collTickets, true)) { // only add it if the **same** object is not already associated
			array_push($this->collTickets, $l);
			$l->setEvent($this);
		}
	}
	...
	public function getTicketsJoinUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		if ($this->collTickets === null) {
			if ($this->isNew()) {
				...
				$this->collTickets = TicketPeer::doSelectJoinUser($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
